// build.js for creators dashboard
...
async function modifyHTMLForSubpath(htmlPath) {
    ...
    // Update all absolute paths to use /creators prefix
    content = content.replace(/src="\/assets\//g, 'src="/creators/assets/');
    content = content.replace(/href="\/assets\//g, 'href="/creators/assets/');
    content = content.replace(/from "\/assets\//g, 'from "/creators/assets/');

    // Update the base path for React Router
    content = content.replace(
        '<div id="root"></div>',
        '<div id="root" data-base-path="/creators"></div>'
    );
...
async function modifyJSForSubpath(jsPath) {
    ...
    content = content.replace(
        '<BrowserRouter>',
        '<BrowserRouter basename="/creators">'
    );
...
async function main() {
    console.log('🚀 Starting Creators Dashboard build process...\n');
    ...
    try {
        ...
        // Step 3: Copy built files to public/creators
        console.log('📁 Copying built files...');
        const distPath = path.join(__dirname, 'dist');

        // Copy all files from dist to public/creators
        await copyDirectory(distPath, outputDir);
        console.log('✅ Files copied to public/creators\n');

        // Step 4: Modify index.html for subpath
        console.log('🔧 Modifying files for /creators subpath...');
