...
async function modifyHTMLForSubpath(htmlPath) {
    ...
    // Update all absolute paths to use /store prefix
    content = content.replace(/src="\/assets\//g, 'src="/store/assets/');
    content = content.replace(/href="\/assets\//g, 'href="/store/assets/');
    content = content.replace(/from "\/assets\//g, 'from "/store/assets/');

    // Update the base path for React Router
    content = content.replace(
        '<div id="root"></div>',
        '<div id="root" data-base-path="/store"></div>'
    );
...
async function main() {
    ...
    const outputDir = path.join(__dirname, '..', '..', 'public', 'store');
    ...
    try {
        ...
        // Step 3: Copy built files to public/store
        console.log('📁 Copying built files...');
        const distPath = path.join(__dirname, 'dist');

        // Copy all files from dist to public/store
        await copyDirectory(distPath, outputDir);
        console.log('✅ Files copied to public/store\n');

        // Step 4: Modify index.html for subpath
        console.log('🔧 Modifying files for /store subpath...');
        ...
        console.log('🎉 VAPR Store build completed successfully!');
