...
async function modifyHTMLForSubpath(htmlPath) {
    ...
    // Update all absolute paths to use /library prefix
    content = content.replace(/src="\/assets\//g, 'src="/library/assets/');
    content = content.replace(/href="\/assets\//g, 'href="/library/assets/');
    content = content.replace(/from "\/assets\//g, 'from "/library/assets/');

    // Update the base path for React Router
    content = content.replace(
        '<div id="root"></div>',
        '<div id="root" data-base-path="/library"></div>'
    );
...
async function main() {
    ...
    const outputDir = path.join(__dirname, '..', '..', 'public', 'library');
    ...
    try {
        ...
        // Step 3: Copy built files to public/library
        console.log('📁 Copying built files...');
        const distPath = path.join(__dirname, 'dist');

        // Copy all files from dist to public/library
        await copyDirectory(distPath, outputDir);
        console.log('✅ Files copied to public/library\n');

        // Step 4: Modify index.html for subpath
        console.log('🔧 Modifying files for /library subpath...');
