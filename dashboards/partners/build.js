...
async function modifyHTMLForSubpath(htmlPath) {
    ...
    // Update all absolute paths to use /partners prefix
    content = content.replace(/src="\/assets\//g, 'src="/partners/assets/');
    content = content.replace(/href="\/assets\//g, 'href="/partners/assets/');
    content = content.replace(/from "\/assets\//g, 'from "/partners/assets/');

    // Update the base path for React Router
    content = content.replace(
        '<div id="root"></div>',
        '<div id="root" data-base-path="/partners"></div>'
    );
...
async function main() {
    console.log('🚀 Starting Partners Dashboard build process...\n');

    const outputDir = path.join(__dirname, '..', '..', 'public', 'partners');
    ...
    try {
        ...
        // Step 3: Copy built files to public/partners
        console.log('📁 Copying built files...');
        const distPath = path.join(__dirname, 'dist');

        // Copy all files from dist to public/partners
        await copyDirectory(distPath, outputDir);
        console.log('✅ Files copied to public/partners\n');
