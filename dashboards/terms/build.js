...
async function modifyHTMLForSubpath(htmlPath) {
    ...
    // Update all absolute paths to use /terms prefix
    content = content.replace(/src="\/assets\//g, 'src="/terms/assets/');
    content = content.replace(/href="\/assets\//g, 'href="/terms/assets/');
    content = content.replace(/from "\/assets\//g, 'from "/terms/assets/');

    // Update the base path for React Router
    content = content.replace(
        '<div id="root"></div>',
        '<div id="root" data-base-path="/terms"></div>'
    );
...
async function main() {
    console.log('🚀 Starting terms build process...\n');

    const outputDir = path.join(__dirname, '..', '..', 'public', 'terms');
    ...
    try {
        ...
        // Step 3: Copy built files to public/terms
        console.log('📁 Copying built files...');
        const distPath = path.join(__dirname, 'dist');

        // Copy all files from dist to public/terms
        await copyDirectory(distPath, outputDir);
        console.log('✅ Files copied to public/terms\n');

        // Step 4: Modify index.html for subpath
        console.log('🔧 Modifying files for /terms subpath...');
