...
async function modifyHTMLForSubpath(htmlPath) {
    ...
    // Update all absolute paths to use /privacy prefix
    content = content.replace(/src="\/assets\//g, 'src="/privacy/assets/');
    content = content.replace(/href="\/assets\//g, 'href="/privacy/assets/');
    content = content.replace(/from "\/assets\//g, 'from "/privacy/assets/');

    // Update the base path for React Router
    content = content.replace(
        '<div id="root"></div>',
        '<div id="root" data-base-path="/privacy"></div>'
    );
...
async function main() {
    console.log('🚀 Starting Privacy build process...\n');

    const outputDir = path.join(__dirname, '..', '..', 'public', 'privacy');
    ...
    try {
        ...
        // Step 3: Copy built files to public/privacy
        console.log('📁 Copying built files...');
        const distPath = path.join(__dirname, 'dist');

        // Copy all files from dist to public/privacy
        await copyDirectory(distPath, outputDir);
        console.log('✅ Files copied to public/privacy\n');

        // Step 4: Modify index.html for subpath
        console.log('🔧 Modifying files for /privacy subpath...');
        ...
        console.log('🎉 Privacy build completed successfully!');
